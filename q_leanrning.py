...
class QLearning:
    ...
    def discretize_state(self, paddle_position, ball_position):
        paddle_bin = int(np.digitize(paddle_position, np.linspace(0, HEIGHT, self.num_bins_paddle)))
        ball_x_bin = int(np.digitize(ball_position[0], np.linspace(0, WIDTH, self.num_bins_ball_x)))
        ball_y_bin = int(np.digitize(ball_position[1], np.linspace(0, HEIGHT, self.num_bins_ball_y)))
...
def main():
    ...
    # variable used to track score, starts at 0
    paddle_one_score = 0
    paddle_two_score = 0
    ...
    while running:
        ...
        # variable used to determine if a point has been scored and by what player
        point = ball.update()
        ...
        q_learning.game_state(paddle_one_y_position, paddle_two_y_position, ball_x_and_y_position,
                              paddle_one_score, paddle_two_score)

        q_learning.makeQtables()
