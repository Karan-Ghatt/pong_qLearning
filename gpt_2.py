...
class QLearning:
    ...
    def get_q_values(self, paddle, state):
        if paddle == 'one':
            return self.q_table_paddle_one[state]
        elif paddle == 'two':
            return self.q_table_paddle_two[state]

    def update_q_value(self, paddle, state, action, reward, next_state):
        if paddle == 'one':
            current_q = self.q_table_paddle_one[state][action]
            max_future_q = np.max(self.q_table_paddle_one[next_state])
            new_q = (1 - self.learning_rate) * current_q + \
                    self.learning_rate * (reward + self.discount_factor * max_future_q)
            self.q_table_paddle_one[state][action] = new_q
        elif paddle == 'two':
            current_q = self.q_table_paddle_two[state][action]
            max_future_q = np.max(self.q_table_paddle_two[next_state])
            new_q = (1 - self.learning_rate) * current_q + \
                    self.learning_rate * (reward + self.discount_factor * max_future_q)
            self.q_table_paddle_two[state][action] = new_q

    def choose_action(self, paddle, state):
        if np.random.rand() < self.exploration_rate:
            return np.random.choice(self.num_actions)
        else:
            q_values = self.get_q_values(paddle, state)
...
def main():
    ...
    while running:
        ...
        paddle_one_state = q_learning.discretize_state(paddle_one_y_position, ball_x_and_y_position)
        paddle_two_state = q_learning.discretize_state(paddle_two_y_position, ball_x_and_y_position)

        # choose actions for paddles using Q-learning
        paddle_one_action = q_learning.choose_action('one', paddle_one_state)
        paddle_two_action = q_learning.choose_action('two', paddle_two_state)

        # update paddles based on chosen actions
        paddle_one.update(paddle_one_action - 1)  # actions: 0 (stay), 1 (move down), -1 (move up)
        paddle_two.update(paddle_two_action - 1)
        ...
        # update Q-values based on rewards and next state
        q_learning.update_q_value('one', paddle_one_state, paddle_one_action, 0, paddle_one_state)  # reward is 0
        q_learning.update_q_value('two', paddle_two_state, paddle_two_action, 0, paddle_two_state)

        def discretize_state(self, paddle_position, ball_position):
            paddle_bin = min(int(np.digitize(paddle_position, np.linspace(0, HEIGHT, self.num_bins_paddle))) - 1,
                             self.num_bins_paddle - 1)
            ball_x_bin = min(int(np.digitize(ball_position[0], np.linspace(0, WIDTH, self.num_bins_ball_x))) - 1,
                             self.num_bins_ball_x - 1)
            ball_y_bin = min(int(np.digitize(ball_position[1], np.linspace(0, HEIGHT, self.num_bins_ball_y))) - 1,
                             self.num_bins_ball_y - 1)
            return paddle_bin, ball_x_bin, ball_y_bin
